...
class FCH_IO extends Bundle {
    ...
    val i_ifch    = Input( UInt((params.Parameters.ISAWidth).W))//Instruction Fetch Port

    val o_exe     = Output(Bool())                              //Validation followed Pipeline Stages
    val o_ins     = Output(UInt((params.Parameters.ISAWidth).W))//Instruction Output to Followed Pipe-Stage
}
